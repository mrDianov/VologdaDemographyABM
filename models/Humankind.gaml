...
//		birthday <- current_date; // временная мера
		...
		add self to: humanMap at: self.id;
	}
...
//		}
		int womanId <- nil;
		ask database {
			list<list> women <- getReproductiveWomen()[0];
			if (length(women) > 0 and myself.id > 0) {
				womanId <- int(women[0]);
				if (womanId != nil and womanId != 0) {
					do makePair(myself.id, womanId);
				}
				
			}	
		}
		if (womanId != nil and womanId != 0) {
			do make_pair(humanMap[womanId]);
		}
